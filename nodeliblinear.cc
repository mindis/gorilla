...
void NodeLiblinear::_zeroA(float* a)
{
  int i;
  for (i = 0; i < _dim; i++)
  {
...
void NodeLiblinear::_copyAB(float* a, float* b)
{
  int i;
  for (i = 0; i < _dim; i++)
  {
...
void NodeLiblinear::_addAB(float* a, float* b)
{
  int i;
  for (i = 0; i < _dim; i++)
  {
...
void NodeLiblinear::_subAB(float* a, float* b)
{
  int i;
  for (i = 0; i < _dim; i++)
  {
...
void NodeLiblinear::_addABW(float* a, float* b, float w)
{
  int i;
  for (i = 0; i < _dim; i++)
  {
...
void NodeLiblinear::_scaleA(float* a, float w)
{
  int i;
  for (i = 0; i < _dim; i++)
  {
...
float NodeLiblinear::_dotAB(float* a, float* b)
{
  int i;
  float result = 0;
  for (i = 0; i < _dim; i++)
...
float NodeLiblinear::_normA(float* a)
{
  return _dotAB(a, a);
}

void NodeLiblinear::_swap(int* a, int i, int j)
{
  int tmp = a[i];
  a[i] = a[j];
  a[j] = tmp;
...
void NodeLiblinear::write_float(float value)
{
  fprintf(fp, "%f\n", value);
}

float NodeLiblinear::read_float()
{
  float value;
  fscanf(fp, "%f\n", &value);
  return value;
...
void NodeLiblinear::write_int(int value)
{
  fprintf(fp, "%d\n", value);
}

int NodeLiblinear::read_int()
{
  int value;
  fscanf(fp, "%d\n", &value);
  return value;
...
void NodeLiblinear::write_array(float* a, int dim)
{
  int i;
  if (dim <= 0)
    return;
...
float* NodeLiblinear::read_array(int dim)
{
  int i;
  if (dim <= 0)
    return 0;
...
void NodeLiblinear::write_array_int(int* a, int dim)
{
  if (dim <= 0)
    return;
...
int* NodeLiblinear::read_array_int(int dim)
{
  int i;
  if (dim <= 0)
    return 0;
...
void NodeLiblinear::write_matrix(float** matrix, int num, int dim)
{
  int i;
  if (dim <= 0 || num <= 0)
    return;
...
float** NodeLiblinear::read_matrix(int num, int dim)
{
  int i;
  if (dim <= 0 || num <= 0)
    return 0;
...
void NodeLiblinear::serialize(char* filename)
{
  fp = fopen(filename, "w");
  // dimension
  write_int(_dim);
...
void NodeLiblinear::deserialize(char* filename)
{
  fp = fopen(filename, "r");
  // dimension
  _dim = read_int();
...
void NodeLiblinear::set_dim(int dim)
{
  _dim = dim;
}

int NodeLiblinear::create_data(int num)
{
  int i;
  if (data != 0) return -1;
  data = (GorillaData)malloc(sizeof(struct DataT));
...
void NodeLiblinear::destroy_data()
{
  int i;
  if (data != 0)
  {
...
int NodeLiblinear::is_data_ready()
{
  return (data != 0);
}

void NodeLiblinear::printA(int* a, int dim)
{
  int i;
  for (i = 0; i < dim-1; i++)
  {
...
void NodeLiblinear::split_data(float ratio_train, float ratio_test)
{
...
void NodeLiblinear::random_data()
{
  int i,j;
  for (i = 0; i < data->num; i++)
  {
...
void NodeLiblinear::add_data(float* x, float y, float c)
{
  int i = 0;
...
int NodeLiblinear::create_model(float gamma, float rho, float eps, int max_num_iters)
{
  if (model != 0) return -1;
  model = (GorillaModel)malloc(sizeof(struct LinearModelT));
  model->gamma = gamma;
...
void NodeLiblinear::destroy_model()
{
  if (model != 0)
  {
    _freeA(model->dq);
...
int NodeLiblinear::is_model_ready()
{
  return (model!=0);
}

void NodeLiblinear::reset_model_parameter(float gamma, float rho, float eps, int max_num_iters)
{
  if (is_model_ready() > 0)
  {
    model->gamma = gamma;
...
void NodeLiblinear::reset_model_values()
{
  if (is_model_ready() > 0)
  {
    _zeroA(model->w);
...
void NodeLiblinear::init_solver(int warm_start)
{
  int i, j;
  float* w = model->w;
  _copyAB(w, model->z);
...
void NodeLiblinear::train(float* z, int warm_start)
{
  float r;
  float* dq = model->dq;
  float* q = model->q;
...
void NodeLiblinear::merge(float* dq)
{
...
float NodeLiblinear::predict(float* x)
{
  return _dotAB(model->w, x);
}

void NodeLiblinear::validate()
{
  int i, j;
